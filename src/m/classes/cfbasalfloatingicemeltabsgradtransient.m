...
%
%   Usage:
%      cfbasalfloatingicemeltabsgradtransient=cfdragcoeffabsgradtransient();
%      cfbasalfloatingicemeltabsgradtransient=cfdragcoeffabsgradtransient('name','SurfaceAltimetry',...
%                    'definitionstring','Outputdefinition1',... 
%                    'weights',ones(md.mesh.numberofvertices+1,1))%
%

classdef cfbasalfloatingicemeltabsgradtransient
	properties (SetAccess=public)
		%cfbasalfloatingicemeltabsgrad
		name               = '';
	...
	methods
		...
		function self = cfbasalfloatingicemeltabsgradtransient(varargin) % {{{
			if nargin==0,
		...
		function md = checkconsistency(self,md,solution,analyses) % {{{
			...
			if ~ischar(self.name),
				error('cfbasalfloatingicemeltabsgradtransient error message: ''name'' field should be a string!');
			end
		...
		function md = disp(self) % {{{
			...
			fielddisplay(self,'name','identifier for this cfbasalfloatingicemeltabsgrad response');
			fielddisplay(self,'definitionstring','string that identifies this output definition uniquely, from ''Outputdefinition[1-10]''');
			fielddisplay(self,'weights','weights (at vertices) to apply to the cfbasalfloatingicemeltabsgrad');

		end % }}}
		function md = marshall(self,prefix,md,fid) % {{{

		WriteData(fid,prefix,'data',self.name,'name','md.cfbasalfloatingicemeltabsgradtransient.name','format','String');
		WriteData(fid,prefix,'data',self.definitionstring,'name','md.cfbasalfloatingicemeltabsgradtransient.definitionstring','format','String');
		WriteData(fid,prefix,'data',self.weights,'name','md.cfbasalfloatingicemeltabsgradtransient.weights','format','DoubleMat','mattype',1,'timeserieslength',md.mesh.numberofvertices+1,'yts',md.constants.yts);
		end % }}}
	end
end
